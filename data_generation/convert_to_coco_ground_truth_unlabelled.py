...
SCALE = 4.0
SKIP_INTERVAL = 24
DATASET_NAME = 'new_detection_target_small_unlabelled'
splits = ['train', 'val', 'test']


def unlabelled_datasets(split):
    file_names = []
    if 'target' in DATASET_NAME:
        file_names = VIDEO_CLIPS_TARGET[split+'_unlabelled']
    elif 'source' in DATASET_NAME:
        file_names = VIDEO_CLIPS[split+'_unlabelled']
    json_obj = {
        "categories": [
            {
                "id": 0,
    ...
    for file in file_names:
        frames = get_frames(os.path.join(DATASET_DIR, 'raw_data', 'videos_02', f'{file}.mp4'))
...
unlabelled_datasets('train')
# labelled_datasets('val')
# labelled_datasets('test')
