...
def train_one_epoch(model: torch.nn.Module,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        total_loss = gan_loss_coef * generator_loss
        ...
        total_loss_value = gan_loss_dict_reduced_scaled['loss_generator'].item()
        ...
        metric_logger.update(**gan_loss_dict_reduced_scaled, **gan_loss_dict_reduced_unscaled)
...
@torch.no_grad()
def evaluate(model, discriminator_model, discriminator_criterion, data_loader, base_ds, device, output_dir, batch_size):
    model.eval()
    # criterion.eval()
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        outputs, _, target_features = model(samples, feature_only=True)
        target_features = target_features[-1].tensors
        discriminator_output_target_new = discriminator_model(target_features).view(-1)
        generator_loss = discriminator_criterion(discriminator_output_target_new, true_labels)
        ...
        total_loss_value = gan_loss_dict_reduced_scaled['loss_generator']
