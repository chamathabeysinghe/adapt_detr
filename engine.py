...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    device: torch.device, epoch: int, FL, FL_ENC, max_norm: float = 0, disc_loss_coef=1, disc_loss_1_coef=1, ):
    model.train()
    criterion.train()
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        dloss_total = (dloss_s_p + dloss_t_p) * disc_loss_1_coef + (dloss_s + dloss_t + dloss_enc_s + dloss_enc_t) * disc_loss_coef
        losses += dloss_total
        ...
        metric_logger.update(loss=loss_value, detr_loss=detr_loss_value, **loss_dict_reduced_scaled, **loss_dict_reduced_unscaled)
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        loss_dict_reduced_unscaled = {f'{k}_unscaled': v
                                      for k, v in loss_dict_reduced.items()}
        metric_logger.update(detr_loss=sum(loss_dict_reduced_scaled.values()),
