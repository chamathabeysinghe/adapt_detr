...
def write_video_file(vid_frames, file_name):
...
@torch.no_grad()
def infer(images_path, model, postprocessors, device, output_path, skip=1):
    model.eval()
    duration = 0
    results_data = []
    video_frames = []
    for img_index, img_sample in enumerate(images_path):
        if img_index % skip != 0:
            continue
    ...
    return results_data, video_frames


def process_vid_file(file_name, model, postprocessors, device, args):
    dest_file_directory = os.path.join(args.output_dir, args.exp_name, file_name)
    os.makedirs(dest_file_directory, exist_ok=True)
    images_dir = os.path.join(args.data_path, file_name)
    image_paths = get_images(images_dir)
    data, video_frames = infer(image_paths, model, postprocessors, device, dest_file_directory, args.frame_skip)
    write_video_file(video_frames, os.path.join(args.output_dir, args.exp_name, f'{file_name}-{args.exp_name}.mp4'))
...
if __name__ == "__main__":
    ...
    for vid in video_names:
        ...
        process_vid_file(vid, model, postprocessors, device, args)
