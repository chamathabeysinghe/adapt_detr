...
def get_args_parser():
    ...
    parser.add_argument('--data_path_target', type=str)
...
def get_dataset(video_file, args):
    dataset_val_target = get_video_dataset(video_file, args=args)
    if args.distributed:
    ...
    return data_loader_val_target, base_ds_target
...
def main(args):
    ...
    video_files = VIDEO_CLIPS_TARGET['train'] + VIDEO_CLIPS_TARGET['val'] + VIDEO_CLIPS_TARGET['test']
    output_records = []
    for vid_file in video_files:
        print(f'Evaluating file : {vid_file}')
        data_loader_val_target, base_ds_target = get_dataset(vid_file, args)
        test_stats_target, coco_evaluator_target = evaluate(model, criterion, postprocessors,
                                                            data_loader_val_target, base_ds_target, device,
                                                            args.output_dir)
        output_records.append([vid_file]+list(test_stats_target['coco_eval_bbox']))

    df = pd.DataFrame(output_records)
    df.to_csv('VideosAP_results.csv')
